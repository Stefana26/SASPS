...
export default function RoomDetails() {
    ...
    try {
      ...
      if (response.ok) {
        ...
        Swal.fire({
          ...
          text: errorData.message || "Unable to complete the booking. Please try again.",
          confirmButtonColor: "#2563eb",
        });
      }
    } catch (error) {
  ...
  return (
    <div className="bg-white shadow-lg rounded-2xl p-6 max-w-2xl mx-auto">
      <h2 className="text-3xl font-bold mb-3 text-blue-700">Room {room.roomNumber} — {room.roomType}</h2>
      <p className="text-gray-600 mb-4">{room.description}</p>

      <div className="space-y-2 mb-6">
        <p><b>Price per Night:</b> <span className="text-green-600 text-xl font-bold">${room.pricePerNight}</span></p>
        <p><b>Max Occupancy:</b> {room.maxOccupancy} guests</p>
        <p><b>Floor:</b> {room.floorNumber}</p>
        <p><b>Status:</b> <span className={`font-semibold ${room.status === "AVAILABLE" ? "text-green-600" : "text-red-600"}`}>{room.status}</span></p>
        ...
        <>
          {!showBookingForm ? (
            ...
            <div className="border border-blue-200 rounded-lg p-6 mb-4 bg-blue-50">
              <h3 className="text-xl font-bold mb-4 text-blue-700">Complete Your Reservation</h3>
                ...
                {bookingData.checkInDate && bookingData.checkOutDate && (
                  <div className="bg-green-50 border border-green-200 rounded-lg p-4">
                    <p className="text-sm text-gray-600">
                      <b>Number of Nights:</b> {Math.ceil((new Date(bookingData.checkOutDate) - new Date(bookingData.checkInDate)) / (1000 * 60 * 60 * 24))}
                    </p>
                    <p className="text-lg font-bold text-green-700 mt-2">
                      Total Price: ${(room.pricePerNight * Math.ceil((new Date(bookingData.checkOutDate) - new Date(bookingData.checkInDate)) / (1000 * 60 * 60 * 24))).toFixed(2)}
                    </p>
                  </div>
                )}
      ...
      {!user && room.status === "AVAILABLE" && (
        <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-4 mb-4">
          <p className="text-yellow-800">Please <a href="/login" className="text-blue-600 font-semibold underline">log in</a> as a user to make a reservation.</p>
        </div>
      )}
      ...
      {user && user.role === "ADMIN" && (
        <div className="bg-blue-50 border border-blue-200 rounded-lg p-4 mb-4">
          <p className="text-blue-800">Admin accounts cannot make reservations. This feature is for customers only.</p>
        </div>
      )}
